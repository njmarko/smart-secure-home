...
export class CsrFormComponent implements OnInit {
  ...
  generateCSR(): void {
    ...
    this._certificateService.generateCSR(this.csr)?.subscribe((response) => {
      if (response.ok){
        this.alertResponse("CSR uspesno sacuvan!");
      } else {
  ...
  sendGeneratedCSR(): void {
    ...
    this._certificateService.sendCSR(this.csrPem)?.subscribe((response) => {
      if (response.ok){
        this.alertResponse("CSR uspesno sacuvan!");
      } else {
        this.alertResponse("Neuspeh!");
