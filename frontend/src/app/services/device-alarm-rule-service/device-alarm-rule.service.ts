...
export class DeviceAlarmRuleService {
  constructor(private http: HttpClient) {}

  create(request: CreateAlarmRuleRequest): Observable<AlarmRule> {
    return this.http.post<AlarmRule>(
      `${environment.adminAppUrl}device-alarm-rules`,
      request
    );
  }

  read(page: number, size: number): Observable<PaginatedResponse<AlarmRule>> {
    return this.http.get<PaginatedResponse<AlarmRule>>(
      `${environment.adminAppUrl}device-alarm-rules`,
      {
        params: {
          page: page,
  ...
  deactivate(id: string): Observable<void> {
    return this.http.delete<void>(
      `${environment.adminAppUrl}device-alarm-rules/${id}`
    );
  }
}
